...
public class Server {
    ...
    public Server() {
        try {
            ...
            ss.bind(new InetSocketAddress(5555)); //bind 5555
            ss.configureBlocking(false);    //set not-blocking
            if (selector == null) {
                selector = Selector.open();
            }
            ss.register(selector, SelectionKey.OP_ACCEPT);   //register
        } catch (Exception e) {
            e.printStackTrace();
            close();
        }
            ...
            public void run() {
                try {
                    while (run) {
                        ...
                        if (selector.select(10) == 0) { //select a seletctkey
                            continue;
                        }
