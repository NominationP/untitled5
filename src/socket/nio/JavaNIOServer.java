...
/**
 * 1 channel ===> socket
 * 2    un-blocking read<->write
 *      io accept()  read
 * 3 write read select thread
 * 4 isWritingMap isReadingMap
 *
 * ---------------------------------------
 * java core , java design ,
 * --------------------------------------------
 *
 * socket
 *
 * c - > s -> server 断掉 在处理的过程中不能接受同一个客户端的第二条消息 ， 处理完才能处理第二条         连接管理（请求）（NIO/IO）
 *
 * 加密RSA
 * 注册：用服务器的公钥对密码加密，给服务器传客户端的公钥  返回OK
 *             |
 * 从证书拿（直接给）
 *
 * 登录： 用客户端私钥加密，服务器用之前穿的公钥解密（客户端私钥没了----》怎么让服务器知道（SSL机制））   服务端返回 session ID（表明客户端的身份，时效性session可过期）
 *
 *
 */
